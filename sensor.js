class Sensor{
    constructor(car){
        this.car=car;
        this.rayCount=5;  // Sensor Detectors (beams)
        this.rayLength=150;  // Sensor Range
        this.raySpread=Math.PI/2;  // 180 deg 
    ...
    #getReading(ray,roadBorders,traffic){
        ...
        for(let i=0;i<roadBorders.length;i++){
            ...
            // If Touching borders, add to arr. Else carry on
            if(touch){
                touches.push(touch);
            }
        }
    ...
    draw(ctx){
        for(let i=0;i<this.rayCount;i++){
            ...
            ctx.moveTo(
                this.rays[i][0].x,  // error around here
                this.rays[i][0].y
            );
            ctx.lineTo(
                end.x,
            ...
            ctx.moveTo(
                this.rays[i][1].x,  // error around here
                this.rays[i][1].y
            );
            ctx.lineTo(
                end.x,
